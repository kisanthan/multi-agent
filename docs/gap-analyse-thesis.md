...
| Classification and extraction are model-backed | shared router plus one process-specific extraction agent | ✅ |
...
| Provider-independent abstraction layer (5.1.5), agent/model swappable | `llm/client.py::choose_profile` | ✅ |
...
### G4 -- No automatic cloud escalation of extraction *(intentional)*

The router and both extraction agents now have explicit, independent model
profiles. Automatic provider fallback is deliberately disabled: connection,
authentication, timeout and rate-limit failures stop the process safely.
Schema validation retains its bounded retry and human clarification path.
