...
> the number or reference itself is read by the respective process-specific
> extraction agent (with its configured model profile).
>
...
The runtime has three independent profiles: shared document routing,
payment extraction, and invoice extraction. Each profile selects Ollama,
Google, Anthropic, or OpenAI plus a concrete model. Provider credentials are
central and reused; deterministic lookup and write agents never become model
calls. `MODEL_MODE` remains only as a compatibility default for older `.env`
files which do not yet contain explicit profiles.

The split is visible in the executable graph: routing determines only the
document type, then the selected process runs its own extraction profile. A
case snapshots provider, model, and configuration revision at start. Provider
failure stops safely without an implicit fallback, while schema violations
retain the deterministic retry and human-escalation path.

The prototype loads no local models itself; it connects to an Ollama instance
(`OLLAMA_BASE_URL`) whose models are provisioned outside it. The settings page
and `demo.py --check` inspect provisioning without making a billable response.
...
Model IDs are discovered through the configured provider connection and can
also be entered manually. The legacy local default remains
`qwen2.5vl:7b`; cloud model IDs are deliberately configuration rather than
architecture because availability changes independently of the workflow.
